<template>
	<view class="uni-padding-wrap">
		...
		<view class="pl15 col3 header-title" style="padding: 0 12px;box-sizing: border-box;"
			:style='{"backgroundColor":"#3A78F1","borderColor":"#06A9FF","color":"rgba(255, 255, 255, 1)","borderRadius":"200rpx","borderWidth":"4rpx","fontSize":"32rpx","lineHeight":"72rpx","borderStyle":"dashed"}'>
		...
		<view class="list" :style='{"borderRadius":"0","backgroundColor":"#efefef"}'>
			<view :style='{"borderRadius":"8rpx","backgroundColor":"#06A9FF"}' @tap="onDetailTap('news',product.id)"
				v-for="(product,index) in newsList" :key="index" class="listm flex flex-between">
				<view class="listmr">
					<view class="col3 f30 elip mb15"
						:style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"#fff","textAlign":"left"}'>
...
<script>
	...
	export default {
		...
		async onShow() {
			...
			let xueshengqingjia = await this.$api.list('xueshengqingjia', {
				page: 1,
				limit: 6,
			});
			this.xueshengqingjiaList = xueshengqingjia.data.list


			let jiaqiweizhi = await this.$api.list('jiaqiweizhi', {
				page: 1,
				limit: 6,
			});
			this.jiaqiweizhiList = jiaqiweizhi.data.list
